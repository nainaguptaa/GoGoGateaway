...
export default function SignUpReminder({ setPopup }) {
  ...
  return (
    <div className="absolute left-0 top-0 z-40 flex h-screen w-full items-center justify-center bg-black/80 backdrop-blur">
      <div className="absolute right-10 top-10">
        <IoMdClose
          size={35}
          onClick={() => setPopup((prev) => !prev)}
          className="ease cursor-pointer text-white transition duration-100 hover:brightness-125"
        />
      </div>
      <div className=" flex w-4/5 items-center gap-8 p-5">
        <div className="flex flex-col gap-2 text-white">
          <div className="text-4xl font-bold ">Hold on a second...</div>
        ...
        <div className="flex flex-col gap-4 rounded-xl border-2 border-background bg-gray-700/30 p-4 text-white">
